...
import { useGetPurchaseReportQuery, useGetPurchaseSummaryQuery } from '@/features/reports/services/reportsApi';
import type { PurchaseOrderRow } from '@/features/reports/types';

const API_URL = process.env.NEXT_PUBLIC_API_URL ?? 'http://localhost:5000/api/v1';
const STATUSES = ['', 'DRAFT', 'CONFIRMED', 'RECEIVED', 'CLOSED'];
...
export default function PurchaseReportPage() {
  ...
  const { data, isLoading, isError, refetch } = useGetPurchaseReportQuery(params);
  const { data: summaryData } = useGetPurchaseSummaryQuery({ from: from || undefined, to: to || undefined });

  const summary = summaryData?.data;
  const totalOrders = summary?.orderSummary.reduce((s, r) => s + r.count, 0) ?? 0;
  const totalAmount = summary?.orderSummary.reduce((s, r) => s + r.totalAmount, 0) ?? 0;

  const csvParams = new URLSearchParams({ format: 'csv', ...(status && { status }), ...(from && { from }), ...(to && { to }) });
  const csvUrl = `${API_URL}/reports/purchase?${csvParams}`;

  const columns: Column<PurchaseOrderRow>[] = [
    { key: 'poNumber', header: 'PO #', priority: 'P2', render: (r) => <span className="font-mono text-[13px]">{r.poNumber}</span> },
    { key: 'supplier', header: 'Supplier', priority: 'P1', render: (r) => <span className="font-medium">{r.supplier?.name}</span> },
  ...
  return (
    <>
      <PageHeader
        ...
        description="Purchase orders, receipts, and supplier dues"
        breadcrumbs={[{ label: 'Reports' }, { label: 'Purchase' }]}
        actions={
      ...
      {summary && (
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-6">
          {[
            { label: 'Total POs', value: totalOrders.toString() },
            { label: 'Total Value', value: formatCurrency(totalAmount) },
            { label: 'Goods Received', value: formatCurrency(summary.receivingSummary.totalAmount) },
          ].map(({ label, value }) => (
      ...
      {summary?.supplierDues && summary.supplierDues.length > 0 && (
        <div className="bg-white rounded-lg border border-border p-4 mb-6">
          <h2 className="text-sm font-semibold text-foreground mb-3">Supplier Outstanding Dues</h2>
          <div className="space-y-2">
            {summary.supplierDues.map((s) => (
              <div key={s._id} className="flex items-center justify-between text-sm">
                <span className="text-foreground">{s.name}</span>
                <span className="font-medium text-red-600">{formatCurrency(s.balance)}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      {isError ? <ErrorState onRetry={refetch} />
        : data?.data?.orders?.length === 0 && !isLoading ? <EmptyState title="No purchase orders" description="No orders match the selected filters." />
