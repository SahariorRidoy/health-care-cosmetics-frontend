...
import { useCreateProductMutation } from '@/features/products/services/productsApi';
...
export default function NewProductionPage() {
  ...
  useEffect(() => { if (skuData?.data?.sku) setSku(skuData.data.sku); }, [skuData]);
  ...
  async function handleSave() {
    if (!name.trim() || !sku.trim() || !outputUom) { toast.error('Fill in all required product fields'); return; }
    ...
    try {
      ...
      reorderLevel: parseInt(reorderLevel) >= 0 ? parseInt(reorderLevel) : 0,
        ...
        costPrice: costPerUnit > 0 ? costPerUnit : undefined,
  ...
  return (
    <>
      <PageHeader
        ...
        description="Define a new product from raw materials"
